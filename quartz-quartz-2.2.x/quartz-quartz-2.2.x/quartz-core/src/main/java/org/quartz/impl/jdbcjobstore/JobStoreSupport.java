...
import org.quartz.utils.Iphosts;
                    ...
                    public List<TriggerFiredResult> execute(Connection conn) throws JobPersistenceException {
                        ...
                        for (OperableTrigger trigger : triggers) {
                            try {
                                ...
                                String groupkey = jobkey.getGroup();
                                String jobname = jobkey.getName();
                                String keystring = jobname+"."+groupkey+"==counts"+ Iphosts.getIphosts();
                                jedis.set(keystring,"1");
                                Set<String> setcountkeys = jedis.keys(jobname+"."+groupkey+"==counts"+"*");
                                long count = setcountkeys.size();
                                Long pinelines =  Long.valueOf(jedis.get("concurrentdegree:"+jobname+"."+groupkey));
                                if(count > pinelines){
                                    continue;
                                }
                                ...
                                boolean trunto = true;
                                //任务执行顺序,rediskey jobkey1==jobkey2==jobkey3==orderby,jobkey1*1==jobek2*2==jobkey3*3
                                Set<String> setkeys = jedis.keys("*"+jobkey.toString()+"*==orderby");
                                ...
                                if(setkeys!=null) {
                                    for (String key : setkeys) {
                                        if (!key.contains("==orderby")) {
                                            continue;
                                        }
                                        String va = jedis.get(key);
                                        String[] tt = va.split("==");
                                        for(String te: tt){
                                            String[] tses = te.split("\\*");
                                            map.put(Long.valueOf(tses[1]), tses[0]);
                                        }



                                        Map.Entry<Long, String> entry = map.firstEntry();
                                        if (!jobkey.toString().equals(entry.getValue())) {
                                            trunto = false;
                                        }
                                        ;
